...
export async function POST(request: Request) {
  ...
  if (q.status === "accepted") return Response.json({ error: "Offerte is al geaccepteerd" }, { status: 400 });
  if (q.status === "expired") return Response.json({ error: "Offerte is verlopen" }, { status: 400 });
